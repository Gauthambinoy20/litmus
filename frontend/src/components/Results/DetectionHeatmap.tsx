    ...
    <div className="card p-5 animate-in">
      ...
      <div className="flex items-center gap-4 mb-4 text-[10px] text-muted">
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-sm bg-success/30" /> Clean (0-0.2)
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-sm bg-warning/30" /> Mild (0.2-0.5)
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-sm bg-orange-400/30" /> Suspicious (0.5-0.75)
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-3 h-3 rounded-sm bg-danger/30" /> Flagged (0.75-1.0)
        </span>
      </div>
          ...
          <div key={idx} className="relative">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`shrink-0 text-[10px] font-semibold ${getRiskTextColor(item.risk)}`}
                >
              ...
              <div className="absolute z-20 left-4 top-full mt-1 px-3 py-2 bg-surface-light text-text rounded-lg shadow-lg border border-border max-w-sm">
                ...
                <ul className="space-y-0.5">
                  {item.flags.map((flag, fi) => (
                    <li key={fi} className="text-[10px] text-text-secondary">
                      - {flag}
                    </li>
                  ))}
                </ul>
