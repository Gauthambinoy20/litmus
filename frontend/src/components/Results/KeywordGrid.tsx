        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {ats.matched_keywords.map((kw) => (
              <span
                key={kw.keyword}
                className="px-2 py-0.5 text-[10px] rounded-md bg-success-dim text-success/90 mono"
              >
                {kw.keyword}
              </span>
            ))}
        ...
        <div className="mt-4 pt-3 border-t border-border">
          ...
          <div className="flex flex-wrap gap-1.5">
            {ats.skills_only_keywords.map((kw) => (
              <span
                key={kw}
                className="px-2 py-0.5 text-[10px] rounded-md bg-warning-dim text-warning/80 mono"
              >
                {kw}
              </span>
            ))}
