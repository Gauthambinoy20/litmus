...
export function WordAnalytics({ analytics, readability }: Props) {
  ...
  const stats = [
    ...
    {
      label: "Sentences",
      value: analytics?.sentence_count ?? readability?.sentence_count ?? 0,
      icon: BarChart3,
    },
    {
      label: "Avg Sentence",
      value: `${(analytics?.avg_sentence_length ?? readability?.avg_sentence_length ?? 0).toFixed(1)} words`,
      icon: BarChart3,
    },
    {
      label: "Vocabulary",
      value: `${((analytics?.vocabulary_richness ?? readability?.vocabulary_richness ?? 0) * 100).toFixed(1)}%`,
      icon: BookOpen,
    },
    ...(readability
      ? [
          {
            label: "Reading Time",
            value: formatTime(readability.reading_time_seconds),
            icon: Clock,
          },
        ]
      : []),
      ...
      {readability && (
        <div className="bg-bg rounded-xl border border-border px-4 py-3 mb-4">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-xs font-medium ${getGradeColor(readability.flesch_kincaid_grade)}`}
            >
