    ...
    <div className="card p-5">
          ...
          >
              ...
              {b.flags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1">
                  {b.flags.map((f, j) => (
                    <span
                      key={j}
                      className="text-[9px] px-1.5 py-0.5 rounded bg-danger-dim text-danger/80"
                    >
                      {f}
                    </span>
                  ))}
            ...
            <div className="flex items-center gap-3 shrink-0 text-[10px] text-muted">
              ...
              {b.diff_from_previous !== null && (
                <span
                  className={`mono w-6 text-right ${b.diff_from_previous < 3 ? "text-warning" : ""}`}
                >
                  {b.diff_from_previous === 0
                    ? "="
                    : `${b.diff_from_previous > 0 ? "+" : ""}${b.diff_from_previous}`}
                </span>
              )}
      ...
      <div className="flex items-center gap-4 mt-4 pt-3 border-t border-border text-[10px] text-muted">
        <span className="flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-success" /> Clean
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-warning" /> Suspicious
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-1.5 h-1.5 rounded-full bg-danger" /> Flagged
        </span>
      </div>
    </div>
  );
