    ...
    <div className="bg-bg rounded-lg border border-border px-3 py-2.5 space-y-1.5">
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          {issue.suggestion && (
            <p className="text-[10px] text-success mt-1">Fix: {issue.suggestion}</p>
          )}
        </div>
        <span
          className={`text-[9px] font-medium px-1.5 py-0.5 rounded ${config.bg} ${config.color} shrink-0`}
        >
          {issue.type}
        </span>
      </div>
    ...
    <div className="border border-border rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-[10px] font-semibold mono px-1.5 py-0.5 rounded ${config.bg} ${config.color}`}
          >
            {issues.length}
          </span>
        </div>
  ...
  return (
    <div className="card p-5 animate-in">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="mono text-lg font-bold" style={{ color: scoreColor }}>
      ...
      <div className="flex items-center gap-2 mb-4">
        {grammar.error_count > 0 && (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-danger/10 text-danger rounded-full text-[10px] font-medium">
            <AlertCircle size={10} /> {grammar.error_count} error
            {grammar.error_count !== 1 ? "s" : ""}
          </span>
        )}
        {grammar.warning_count > 0 && (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-warning/10 text-warning rounded-full text-[10px] font-medium">
            <AlertTriangle size={10} /> {grammar.warning_count} warning
            {grammar.warning_count !== 1 ? "s" : ""}
          </span>
        )}
        {grammar.suggestion_count > 0 && (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-primary/10 text-primary rounded-full text-[10px] font-medium">
            <Lightbulb size={10} /> {grammar.suggestion_count} suggestion
            {grammar.suggestion_count !== 1 ? "s" : ""}
          </span>
        )}
