  ...
  return (
    <div className="card p-5">
      <div className="flex items-center justify-between mb-5">
        ...
        <span
          className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ${riskColors[ai.risk_level] || ""}`}
        >
          {ai.risk_level}
        </span>
      </div>
          ...
          return (
            <div key={s.name}>
              ...
              >
                ...
                <span className="mono text-[10px] text-muted w-7 text-right shrink-0">
                  {s.score.toFixed(1)}
                </span>
              </button>
                  ...
                  {s.flagged_items.length > 0 && (
                    <div className="mt-1.5 space-y-0.5">
                      {s.flagged_items.slice(0, 5).map((item, i) => (
                        <p key={i} className="text-[10px] text-danger/70 mono">
                          {item}
                        </p>
                      ))}
                    </div>
