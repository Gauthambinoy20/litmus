...
export function BulkUpload({ jdText, contentMode }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Error */}
      {fileError && <p className="text-[11px] text-danger px-1">{fileError}</p>}
      ...
      >
        ...
        <td className="px-3 py-2.5 text-center">
          {result ? (
            <span
              ...
              style={{ color: getScoreColor(result.ats_score.overall_score) }}
            >
        ...
        <td className="px-3 py-2.5 text-center">
          {result ? (
            <span
              className="mono font-semibold"
              style={{ color: getScoreColor(result.ai_score.overall_score, true) }}
            >
