...
export function ScannerPage() {
  ...
  const triggerQuickScore = useCallback(
    (text: string) => {
  ...
  return (
    <section id="scanner" className="max-w-5xl mx-auto px-6 py-16" onKeyDown={handleKeyDown}>
      ...
      <div
        className={`grid ${isResumeMode && inputMode !== "bulk" ? "md:grid-cols-2" : "md:grid-cols-1"} gap-5 mb-5`}
      >
        <div className="relative">
          ...
          {inputMode === "text" && resumeText.length >= 100 && (
            <div className="absolute top-12 right-2 z-10">
              <div
                className="flex items-center justify-center w-12 h-12 rounded-full border-2 bg-bg shadow-lg"
                style={{
                  borderColor: quickScoreLoading
                    ? "#6366F1"
                    : quickScore !== null
                      ? getQuickScoreColor(quickScore)
                      : "#6366F1",
                }}
              >
                {quickScoreLoading ? (
                  ...
                  <span
                    ...
                    style={{ color: getQuickScoreColor(quickScore) }}
                  >
      ...
      {inputMode !== "bulk" && (
        <ScanButton onClick={handleScan} isLoading={isLoading} disabled={!canScan} />
      )}
      ...
      <div className="flex items-center justify-center gap-4 mt-3">
        <p className="text-[10px] text-muted">
          <kbd className="px-1 py-0.5 bg-surface border border-border rounded text-[9px]">Ctrl</kbd>{" "}
          +{" "}
