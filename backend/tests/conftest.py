...
def block_external_http(monkeypatch):
    """Tests must never hit real external APIs (HuggingFace, etc.).

    Simulate the offline path so engines exercise their graceful fallbacks
    deterministically and for free."""

    def _refuse(*args, **kwargs):
        raise httpx.ConnectError("external HTTP disabled in tests")
    ...
    monkeypatch.setattr("app.engines.ai_detection_engine.httpx.post", _refuse)
    monkeypatch.setattr("app.engines.humanizer_engine.httpx.AsyncClient", _RefusingAsyncClient)
