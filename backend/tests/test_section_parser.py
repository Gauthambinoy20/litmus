...
def test_infers_sections_from_headerless_text(parser):
    # No explicit headers, but education/skills cues are present —
    # the parser infers sections instead of falling back to flat-text mode.
    ...
    assert result.flat_text_mode is False
    assert "Education" in result.sections_found


def test_handles_no_sections_gracefully(parser):
    # Text with no headers AND no inferable cues falls back to flat-text mode.
    text = """Hello there. This is some plain prose about cooking dinner at home.
    Yesterday it rained all afternoon and the garden flooded a little.
    The cat slept through the whole storm without noticing anything at all."""
    result = parser.parse(text)
