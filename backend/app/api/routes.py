...
from app.engines.constants import AI_STRUCTURE_PATTERNS, BANNED_EXTENDED_WORDS, BANNED_PHRASES
...
router = APIRouter(prefix="/api/v1", tags=["Litmus"])
...
@router.post(
    ...
    description=(
        "Rewrites AI-flagged resume text to be undetectable by AI detectors "
        "using layered rewriting (ML paraphrase + rule-based)"
    ),
    responses={400: {"model": ErrorResponse}, 429: {"model": ErrorResponse}},
)
