...
class ATSEngine:
    ...
    def score(
        ...
        try:
            ...
            kw_score, matched, missing = self._score_keyword_match(resume_lower, keywords, sections)
            result.keyword_match_score = kw_score
            result.matched_keywords = matched
            result.missing_keywords = missing

            # 2. Keyword Placement Quality (20%)
            placement_score, skills_only = self._score_keyword_placement(matched, sections)
