...
from app.engines.section_parser import BulletPoint, SectionParseResult
...
class AIDetectionEngine:
    ...
    def analyze(
        ...
        try:
            ...
            result.top_issues = [f"{s.name}: {s.details}" for s in sorted_signals[:5] if s.score > 0]
    ...
    def _signal_burstiness(self, bullets: list[BulletPoint]) -> DetectionSignal:
        ...
        if burstiness > -0.3 and change_rate < 0.35:
            score = MAX_SIGNAL_SCORE
            details = (
                f"Very low burstiness ({burstiness:.2f}) with monotone rhythm ({change_rate:.0%} changes). AI-typical."
            )
    ...
    def _signal_passive_voice_density(self, text: str) -> DetectionSignal:
        ...
        if passive_ratio < 0.02 and total_sentences > 5:
            score = MAX_SIGNAL_SCORE * 0.7
            details = (
                f"100% active voice across {total_sentences} sentences. AI-typical (humans occasionally use passive)."
            )
    ...
    def _signal_perplexity_estimate(self, text: str) -> DetectionSignal:
        ...
        observed_freqs: dict[str, float] = {tri: count / total_trigrams * 100 for tri, count in text_trigrams.items()}
