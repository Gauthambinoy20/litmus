...
from app.engines.constants import JD_DETECTION_PHRASES, RECOMMENDED_SECTIONS, REQUIRED_SECTIONS, RESUME_SECTIONS
...
class SectionParser:
    def parse(self, resume_text: str) -> SectionParseResult:
        ...
        if jd_matches >= 3:
            result.warnings.append(
                "This text appears to be a job description, not a resume. " "Make sure you haven't swapped the inputs."
            )
    ...
    def _infer_sections_from_content(self, lines: list[str]) -> list[tuple[int, str]]:
        ...
        for i, line in enumerate(lines):
            ...
            if date_pattern.search(stripped) and _BULLET_PATTERN.match(
                lines[min(i + 1, len(lines) - 1)].strip() if i + 1 < len(lines) else ""
            ):
